                ...
                <tbody>
                  <tr v-for="request in pendingRequests" :key="request.request_id">
                    ...
                    <td>
                      ...
                      <span v-else="request.is_recurring" class="badge bg-primary ms-1">Ad-hoc</span>
                    </td>
                  </tr>
                </tbody>
              </table>
...
const fetchPendingRequests = async () => {
  try {
    const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/pending-requests/${userData.staff_id}`);
