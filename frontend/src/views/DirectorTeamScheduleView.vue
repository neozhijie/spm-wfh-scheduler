<template>
            ...
            <div v-if="selectedDateDetails" class="details-container">
              ...
              <div class="tab-content">
                <div v-if="activeTab === 'AM'">
                  ...
                  <div v-if="selectedDateDetails.type === 'direct'">
                  ...
                  <div v-else-if="selectedDateDetails.type === 'manager'">
                    <div class="teams">
                      <div 
                        class="team card" 
                        v-for="(managerTeam, managerName) in selectedDateDetails.AM.teams" 
                        :key="managerName"
                      ...
                      >
                        <div class="team-header">
                          ...
                          <div class="badge-container">
                            ...
                            <span class="badge badge-wfh">
                              WFH: {{ managerTeam.wfh }}/{{ managerTeam.total }}
                            </span>
                          </div>
                        </div>
                        <div class="sub-teams" v-if="expandedTeams['AM-' + managerName]">
                          <div 
                            class="sub-team card" 
                            v-for="team in managerTeam.subTeams" 
                            :key="team.teamName"
                            @click.stop="toggleSubTeam('AM', managerName, team.teamName)"
                          >
                            <div class="team-header">
                              <h4>{{ team.teamName }}</h4>
                              <div class="badge-container">
                            ...
                            <div class="staff-list" v-if="expandedTeams['AM-' + managerName + '-' + team.teamName]">
                              <div class="staff-card" v-for="staff in team.staff" :key="staff.staff_id">
                                <span :class="['status-indicator', staff.status === 'OFFICE' ? 'green' : 'yellow']"></span>
                                <span class="staff-name">{{ staff.name }}</span>
                              </div>
                ...
                <div v-else-if="activeTab === 'PM'">
                  ...
                  <div v-else-if="selectedDateDetails.type === 'manager'">
                    <div class="teams">
                      <div 
                        class="team card" 
                        v-for="(managerTeam, managerName) in selectedDateDetails.PM.teams" 
                        :key="managerName"
                      ...
                      >
                        ...
                        <div class="sub-teams" v-if="expandedTeams['PM-' + managerName]">
                          <div 
                            class="sub-team card" 
                            v-for="team in managerTeam.subTeams" 
                            :key="team.teamName"
                            @click.stop="toggleSubTeam('PM', managerName, team.teamName)"
                          >
                            <div class="team-header">
                              <h4>{{ team.teamName }}</h4>
                              <div class="badge-container">
                            ...
                            <div class="staff-list" v-if="expandedTeams['PM-' + managerName + '-' + team.teamName]">
                              <div class="staff-card" v-for="staff in team.staff" :key="staff.staff_id">
                                <span :class="['status-indicator', staff.status === 'OFFICE' ? 'green' : 'yellow']"></span>
                                <span class="staff-name">{{ staff.name }}</span>
                              </div>
  ...
  async function fetchManagerScheduleSummary(start, end) {
    try {
      ...
      data.forEach(item => {
        ...
        const wfhCountAm = item.wfh_count_am;
        const wfhCountPm = item.wfh_count_pm;
        ...
        newEvents.push({
          title: `(AM) Office: ${officeCountAm}/${totalStaff}, WFH: ${wfhCountAm}/${totalStaff}`,
        ...
        newEvents.push({
          title: `(PM) Office: ${officeCountPm}/${totalStaff}, WFH: ${wfhCountPm}/${totalStaff}`,
    ...
    return {
      ...
      eventContent: function(arg) {
        if (arg.event.extendedProps.timeOfDay) {
          ...
          const wfhCount = arg.event.extendedProps.wfhCount;
  ...
  async function displayDateDetails(dateStr) {
    ...
    if (detailData) {
      ...
      if (detailData.staff) {
        // 'direct' case: process staff list, group by positions
        groupedData.type = 'direct';
        let totalStaff = detailData.staff.length;
        groupedData.AM.total = totalStaff;
        groupedData.PM.total = totalStaff;
      ...
      } else if (detailData.managers) {
        // 'manager' case
        groupedData.type = 'manager';

        // For each manager
        for (let managerTeamName in detailData.managers) {
          const managerData = detailData.managers[managerTeamName];
          const managerName = managerData.manager_name;
          const managerPosition = managerData.manager_position || 'Unknown';
          const staffList = managerData.staff;

          // Initialize team for manager
          ['AM', 'PM'].forEach(period => {
            if (!groupedData[period].teams[managerName]) {
              groupedData[period].teams[managerName] = { teamName: managerName, position: managerPosition, inOffice: 0, wfh: 0, total: 0, subTeams: {} };
            }
          });
          ...
          staffList.forEach(staff => {
            ...
            // AM
            if (!groupedData.AM.teams[managerName].subTeams[position]) {
              groupedData.AM.teams[managerName].subTeams[position] = { teamName: position, inOffice: 0, wfh: 0, total: 0, staff: [] };
            }
            groupedData.AM.teams[managerName].total += 1;
            groupedData.AM.total += 1;
            groupedData.AM.teams[managerName].subTeams[position].total += 1;
            if (staff.status_am === 'OFFICE') {
              groupedData.AM.teams[managerName].inOffice +=1;
              groupedData.AM.inOffice +=1;
              groupedData.AM.teams[managerName].subTeams[position].inOffice += 1;
            } else {
              groupedData.AM.teams[managerName].wfh +=1;
              groupedData.AM.wfh +=1;
              groupedData.AM.teams[managerName].subTeams[position].wfh +=1;
            }
            groupedData.AM.teams[managerName].subTeams[position].staff.push({ staff_id: staff.staff_id, name: staff.name, status: staff.status_am });

            // PM
            if (!groupedData.PM.teams[managerName].subTeams[position]) {
              groupedData.PM.teams[managerName].subTeams[position] = { teamName: position, inOffice: 0, wfh: 0, total: 0, staff: [] };
            }
            groupedData.PM.teams[managerName].total += 1;
            groupedData.PM.total += 1;
            groupedData.PM.teams[managerName].subTeams[position].total += 1;
            if (staff.status_pm === 'OFFICE') {
              groupedData.PM.teams[managerName].inOffice +=1;
              groupedData.PM.inOffice +=1;
              groupedData.PM.teams[managerName].subTeams[position].inOffice += 1;
            } else {
              groupedData.PM.teams[managerName].wfh +=1;
              groupedData.PM.wfh +=1;
              groupedData.PM.teams[managerName].subTeams[position].wfh +=1;
            }
            groupedData.PM.teams[managerName].subTeams[position].staff.push({ staff_id: staff.staff_id, name: staff.name, status: staff.status_pm });
          });
        }
      }
  ...
  // Toggle the expansion state of a sub-team card
  function toggleSubTeam(timeOfDay, teamName, subTeamName) {
    const key = `${timeOfDay}-${teamName}-${subTeamName}`;
    expandedTeams.value[key] = !expandedTeams.value[key];
  }
  </script>
  
  <style scoped>
  /* The styles remain the same as in ManagerTeamScheduleView.vue, you can reuse them */
  .dashboard-container {
    display: flex;
  ...
  /* ... (Include all the styles from ManagerTeamScheduleView.vue) */
...
.team.card:hover {
  ...
  /* Additional styles for sub-teams */
  .sub-team {
    background-color: #ffffff;
    border: 1px solid #dfe3e8;
    border-radius: 8px;
