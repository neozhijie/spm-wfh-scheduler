<template>
  ...
  const departments = ref([]); // List of departments fetched from the backend
  const filteredStaff = ref([]); // Store filtered events
  ...
  async function fetchHRScheduleSummary(start, end) {
    try {
      ...
      console.error('Error fetching HR schedule summary:', error);
    }
  }
  ...
  async function initiateChunkedSummaryLoading() {
    ...
    for (let i = 0; i < ranges.length; i++) {
      ...
      if (i === 0) {
        await fetchHRScheduleSummary(ranges[i].start, ranges[i].end);
      } else {
        setTimeout(async () => {
          await fetchHRScheduleSummary(ranges[i].start, ranges[i].end);
...
function togglePosition(timeOfDay, managerId, positionName) {
  ...
  .custom-dropdown {
    ...
    padding: 5px;
    margin: 5px;
    border: 2px solid #2e3a59; /* Border color */
  ...
  .custom-dropdown:focus {
    ...
    border-color: #2e3a59; /* Change border color on focus */
  }
  </style>